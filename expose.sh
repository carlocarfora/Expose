...
_tmp="${topdir//[!\/]/}"
topdir_slashcount="${#_tmp}"

# if on cygwin, transforms given param to windows style path
# check once at startup rather than on every call
...
do
	...
	_tmp="${node//[!\/]/}"
	node_depth=$(( ${#_tmp} - topdir_slashcount ))
	...
	node_name=$(basename "$node" | sed -e 's/^[0-9]*//' -e 's/^[[:space:]]*//;s/[[:space:]]*$//')
	if [ -z "$node_name" ]
	then
		node_name=$(basename "$node")
	fi
		
	mapfile -t _subdirs < <(find "$node" -maxdepth 1 -type d ! -path "$node" ! -path "$node*/_*")
	dircount="${#_subdirs[@]}"
	dircount_sequence=0
	for _d in "${_subdirs[@]}"; do
		[[ "$_d" != *"$sequence_keyword"* ]] && ((dircount_sequence++))
	done
	...
	then
		...
		if [ ! -z "$sequence_keyword" ] && [[ "$node_name" == *"$sequence_keyword"* ]]
		then
...
do
	...
	url_rel=$(echo "${nav_name[$i]}" | sed 's/[^ a-zA-Z0-9]//g;s/ /-/g;y/ABCDEFGHIJKLMNOPQRSTUVWXYZ/abcdefghijklmnopqrstuvwxyz/')
...
command -v perl >/dev/null 2>&1 && perl_available=true

printf "Building HTML (%ds)\n" $(( SECONDS - phase_start ))
phase_start=$SECONDS
